...
1. Protocol text under `.dds/meta/` is the specification. A change there is a DDS version change: bump `dds_version` in `.dds/tree.dds.md` and `.dds/meta/manifesto.dds.md`, `DDS_VERSION` in `dds.py`, and `metadata.dds_version` in `templates/adapters/skills/dds/SKILL.md` together; the tests check they agree.
