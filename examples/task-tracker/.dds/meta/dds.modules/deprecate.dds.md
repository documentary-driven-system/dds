...
4. IF ALL entries in a local tree become Tombstones (i.e., no active features remain in that domain):
   - DO NOT delete the local tree. MOVE it to the mirrored path under `.dds/archive/` (`.dds/archive/modules/[domain_name]/[domain_name].tree.dds.md`) and mutate its frontmatter like any deprecated document (`status: deprecated`, `deprecated_date`, `original_path`, the DEPRECATED banner). Its Tombstone lines keep pointing at the archived documents.
   - OPEN the parent tree one level up.
   - MUTATE the pointer to the local tree into a Tombstone: `- [DEPRECATED -> .dds/archive/modules/[domain_name]/[domain_name].tree.dds.md]: All features in this domain are deprecated.`
