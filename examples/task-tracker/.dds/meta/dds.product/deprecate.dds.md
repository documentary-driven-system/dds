...
4. IF ALL entries in a local tree become Tombstones (i.e., an entire Epic is canceled):
   - DO NOT delete the local tree. MOVE it to the mirrored path under `.dds/archive/` (`.dds/archive/product/epics/[epic_name]/[epic_name].tree.dds.md`) and mutate its frontmatter like any deprecated document (`status: deprecated`, `deprecated_date`, `original_path`, the DEPRECATED banner). Its Tombstone lines keep pointing at the archived documents.
   - OPEN the parent tree one level up (`product.tree.dds.md`).
   - MUTATE the pointer to the local epic tree into a Tombstone: `- [DEPRECATED -> .dds/archive/product/epics/[epic_name]/[epic_name].tree.dds.md]: This entire Epic has been canceled.`
